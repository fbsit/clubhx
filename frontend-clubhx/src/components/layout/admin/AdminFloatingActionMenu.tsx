...
type AdminFloatingActionMenuProps = {
...
export default function AdminFloatingActionMenu({
  user,
  activePath
}: AdminFloatingActionMenuProps) {
  const [open, setOpen] = useState(false);
  const navigate = useNavigate();
  const adminNavigationGroups = useAdminNavigationGroups();

  // Flatten admin navigation groups into single array
  const navigation: NavigationItem[] = adminNavigationGroups
    .reduce((all, group) => all.concat(group.items), []);

  // Remove "Mi Perfil" from the navigation grid
