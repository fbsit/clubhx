...
type ClientFloatingActionMenuProps = {
...
export default function ClientFloatingActionMenu({
  user,
  activePath
}: ClientFloatingActionMenuProps) {
  const [open, setOpen] = useState(false);
  const navigate = useNavigate();

  // Convert mobileNavigation to NavigationItem format
  const navigation: NavigationItem[] = mobileNavigation.map(item => ({
    ...item,
    icon: renderIcon(item.icon)
  }));

  // Remove "Mi Perfil" from the navigation grid (it's shown in the profile section)
              ...
              <div className="grid grid-cols-2 gap-3 mt-2 mb-2 w-full max-w-xs">
                {filteredNavigation.map((item) => (
                  <button
                    ...
                    className={cn(
                      "flex flex-col items-center justify-center bg-white/70 text-primary/90 hover:text-black active:text-primary transition-all duration-150 group relative",
                      activePath === item.to
